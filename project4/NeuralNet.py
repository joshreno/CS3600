...
class Perceptron(object):
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        # almost the same as sigmoidActivation
        """YOUR CODE"""
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for x in range(len(([1] + inActs[:]))):
            change = alpha * delta * (([1] + inActs[:]))[x]
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        results = [inActs]
        out = []
        for hiddenLayers in range(self.numHiddenLayers):
            outputs = []
            num = self.hiddenLayers[hiddenLayers]
            for x in range(len(num)):
                outputs.append(num[x].sigmoidActivation(results[hiddenLayers]))
            results.append(outputs)

        for x in range(len(self.outputLayer)):
            out.append(self.outputLayer[x].sigmoidActivation(results[-1]))
        results.append(out)
        return results
        """YOUR CODE"""
        ...
        for example in examples:#for each example
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    layers = self.layers[layerNum + 1]
                    delta = gPrime * sum([deltas[0][j] * layers[j].weights[neuronNum + 1] for j in
                                          range(0, len(layers))])
            ...
            for numLayer in xrange(0,self.numLayers):
                ...
                for numNeuron in xrange(len(layer)):
                    ...
                    layers = layer[numNeuron]
                    weightMod = layers.updateWeights(allLayerOutput[numLayer], alpha, deltas[numLayer][numNeuron])
                    averageWeightChange += weightMod
                    numWeights += layers.inSize
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    testError = 0
    testCorrect = 0
    for x in examplesTest:
        feed = nnet.feedForward(x[0])[-1]
        if [round(i) for i in feed] == x[1]:
            testCorrect += 1
        else:
            testError += 1
