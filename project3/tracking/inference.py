...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            emissionDistance = emissionModel[util.manhattanDistance(p, pacmanPosition)]
            if (emissionDistance):
                allPossible[p] = self.beliefs[p] * emissionDistance
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.legalPositions:
                emissionDistance = emissionModel[util.manhattanDistance(p, pacmanPosition)]
                if emissionDistance > 0:
                    allPossible[p] = self.getBeliefDistribution()[p] * emissionDistance
            if allPossible.totalCount() == 0:
                self.initializeUniformly(gameState)
            else :
                for x in range(self.numParticles):
                    self.particles[x] = util.sample(allPossible)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        while self.numParticles > count:
            self.particles.append(allPossible[int((len(allPossible) / (self.numParticles + 0.0)) * count)])
            count += 1
        "*** YOUR CODE HERE ***"
        ...
        for p in self.particles:
            ...
            for ghost in range(self.numGhosts):
                if noisyDistances[ghost] == None:
                    ...
                    count = count * emissionModels[ghost][util.manhattanDistance(p[ghost], pacmanPosition)]
            allPossible[p] = allPossible[p] + count

        if not (allPossible.totalCount()):
            self.initializeParticles()
        if (allPossible.totalCount()):
            self.particles = []
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for ghost in range(self.numGhosts):
                newParticle[ghost] = util.sample(getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), ghost, self.ghostAgents[ghost]))
            "*** END YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        ...
        beliefDistribution = util.Counter()
        for p in self.particles:
            beliefDistribution[p] = beliefDistribution[p] + 1
